		...
		<div class="row">
          ...
          // print_r($data->address); ?>
              ...
              @endforeach
                ...
                {{-- @foreach($data->ward as $c)
                <option value="{{$c->id}}" @if($c->id==$data->address[0]->id_ward) selected @endif >{{$c->ward_name}}</option>
                 @endforeach --}}
						</select>
					</div>
...
{{-- 
					...
					<div class="input-text col-md-12">
						...
						<input type="text" placeholder="Vĩ độ" class="col-md-4" name="place_kinhdo" value="{{$data->info[0]->pl_latitude}}">
						<input type="text" placeholder="Kinh độ" class="col-md-4" name="place_vido" value="{{$data->info[0]->pl_longitude}}">
					</div>
